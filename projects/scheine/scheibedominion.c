...
int adventurerEffect(struct gameState *state, int *drawntreasurePtr, int *currentPlayerPtr, int *cardDrawnPtr, int temphandPtr[], int *zPtr){
	...
	drawCard(*currentPlayerPtr, state);
	*cardDrawnPtr = state->hand[*currentPlayerPtr][state->handCount[*currentPlayerPtr]-1];//top card of hand is most recently drawn card.
	if (*cardDrawnPtr == copper || *cardDrawnPtr == gold || *cardDrawnPtr == gold)
	  (*drawntreasurePtr)++;
	else{
	  temphandPtr[*zPtr]=*cardDrawnPtr;
	  state->handCount[*currentPlayerPtr]--; //this should just remove the top card (the most recently drawn one).
	  (*zPtr)++;
	}
      }
      while(*zPtr-1>=0){
	state->discard[*currentPlayerPtr][state->discardCount[*currentPlayerPtr]++]=temphandPtr[*zPtr-1]; // discard all cards in play that have been drawn
	*zPtr=*zPtr-1;
      }
      return 0;
}
...
int smithyEffect(struct gameState *state, int *currentPlayerPtr, int handPos){
      ...
      for (int i = 0; i < 4; i++)
	{
      ...
      discardCard(handPos, *currentPlayerPtr, state, 0);
      return 0;
}

int council_roomEffect(struct gameState *state, int *currentPlayerPtr, int handPos){
      //+4 Cards
      for (int i = 0; i < 4; i++)
	{
	  drawCard(*currentPlayerPtr, state);
...
int embargoEffect(struct gameState *state, int *currentPlayerPtr, int handPos, int choice1){
      //+2 Coins
...
int villageEffect(struct gameState *state, int *currentPlayerPtr, int handPos){
      //+1 Card
      drawCard(*currentPlayerPtr, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  int *drawntreasurePtr = &drawntreasure;
  int *currentPlayerPtr = &currentPlayer;
  int *temphandPtr = temphand;
  int *cardDrawnPtr = &cardDrawn;
  int *zPtr = &z;
